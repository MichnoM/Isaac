...
isaac_walking_sprites = pygame.image.load('sprites/characters/isaacWalking.png')
isaac_head_sprites = pygame.image.load('sprites/characters/isaacHead.png')
isaac_hurt_sprites = pygame.image.load('sprites/characters/isaacHurt.png')
...
class Player:
    ...
    def draw(self, window):
        if not (self.hurt or self.dead or self.pickup_item):
            sprite = pygame.Surface((32*self.size, 32*self.size), pygame.SRCALPHA)
            ...
            sprite.blit(head_sprite, (0, -head_sprite.get_height()//12))

        else:
            if self.hurt:
                sprite = isaac_hurt_spritesheet.get_image(1, 36, 33, scale=self.size)

            if self.dead:
                sprite = isaac_hurt_spritesheet.get_image(2, 36, 33, scale=self.size)

            if self.pickup_item or self.pickup_pickup:
                sprite = isaac_hurt_spritesheet.get_image(3, 36, 33, scale=self.size)

        sprite = pygame.transform.scale(sprite, (self.width*self.size*2, self.height*self.size*2)).convert_alpha()
    ...
    def update(self):
        ...
        if self.size != self.previous_size:
            previous_center_x = self.x + self.width//2
            previous_center_y = self.y + self.height//2

            self.width = self.width * self.size
            self.height = self.height * self.size

            self.size = self.previous_size
            self.x = previous_center_x - self.width//2
            self.y = previous_center_y - self.height//2
