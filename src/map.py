...
class Map:
    ...
    def draw(self, window):
        ...
        if self.current_room_index == [5, 5]:
            sprite.blit(starting_room_controls_image, (window_width//2 - starting_room_controls_image.get_width()//2, window_height//2 - starting_room_controls_image.get_height()//2))
    ...
    def update(self, character):
        ...
        self.player_info = [character.x, character.y, character.width, character.height]
    ...
    def checkCollision(self, object1, object2, mode = 1):
        '''
        Checks for collision between the two objects, returns object2 if collision detected.
        '''
    ...
    def createTears(self, direction, character, type="friendly", amount=1):
        ...
        if direction == "player":
            delta_x = (self.player_info[0] + self.player_info[2]//2) - (character.x + character.width//2)
            delta_y = (self.player_info[1] + self.player_info[3]//2) - (character.y + character.height//2)
    ...
    def itemEffects(self, item, character):
        if item.name == "piggy bank":
            if character.hurt and character.hurt_interaction:
                self.current_room.itemsSpawn("pickup", character, self, character.x + 60, character.y + 50, "coin")
        ...
        if item.name == "humbling bundle":
            for room in self.rooms:
                room.pickup_quantity = 2

        if item.name == "more options":
            for room in self.rooms:
                if room.type == "treasure":
                    room.amount_of_items = 2
